...
class ImageProcessor:
	...
	def process(self):
		...
		tresh = cv2.adaptiveThreshold(blur, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 57, 7)
		_, binary = cv2.threshold(tresh, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
...
def main(args=None):
	...
	try:
		...
		image_path = input("* | Insert the image path: ").strip() if args is None else args
		...
		treshold_wr = cv2.adaptiveThreshold(blur_wr, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 57, 7)
		_, binary_wr = cv2.threshold(treshold_wr, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
