...
def main(args=None):
	...
	try:
		...
		padding = 1
		width = int(max(np.linalg.norm(rect[0] - rect[1]), np.linalg.norm(rect[2] - rect[3]))) + 2 * padding
		height = int(max(np.linalg.norm(rect[1] - rect[2]), np.linalg.norm(rect[3] - rect[0]))) + 2 * padding
		dst_pts = np.float32([
			[padding, padding],
		...
		_, binary_wr = cv2.threshold(blur_wr, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
		contours_wr, _ = cv2.findContours(binary_wr, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
		largest = max(contours_wr, key=cv2.contourArea)
		contours_wr_filtered = [c for c in contours_wr if cv2.contourArea(c) < cv2.contourArea(largest)]
		cv2.drawContours(warped, contours_wr, -1, (0, 0, 255), 2)
		cv2.drawContours(warped, contours_wr_filtered, -1, (0, 255, 0), 2)
		print(f"Contour detection took {time.time() - t_contour_start:.4f} seconds")

		print(f"Overall runtime: {time.time() - overall_start:.4f} seconds")
